...
| C-3 | **Good Faith Estimate / No Surprises Act disclaimer** — the `NSA_GFE_DISCLAIMER` text (05, `backend/app/agents/cost_estimate.py`) is a faithful CMS-model-language draft carrying a "REQUIRES LEGAL SIGN-OFF BEFORE PRODUCTION" comment. A lawyer / compliance consultant must review and approve the exact wording (it covers: what a GFE is, that it excludes unknown costs, the $400 / 120-day federal dispute right, that it is not a contract, keep a copy), then bump `NSA_GFE_DISCLAIMER_VERSION` — stored estimates snapshot the text + version so old ones are unaffected. | Legal | [PHASE-4 O-3](PHASE-4.md), 05 spec §7 |
